/*
 PecanAva 70cms Tracker
 
 By Anthony Stirk M0UPU 
 Adapted to the PecanPico3 board by Thomas Krahn KT5TK / DL4MDW
...
/* General Configuration (independent from radio) */

#define RADIO SI446X
//#define RADIO RFM22

#define CALLSIGN "PECAN"

#define ASCII 7                        // ASCII 7 or 8
#define STOPBITS 2                     // Either 1 or 2
#define TXDELAY 0                      // Delay between sentence TX's (in 1/50th sec at 50 baud)
#define RTTY_BAUD 50                   // Baud rate; Max = 600
#define RADIO_FREQUENCY 434.550        // Transmit frequency in MHz
#define RTTY_SHIFT      425            // RTTY shift in Hz

#define RADIO_REBOOT 20  // Reboot Radio every X telemetry lines
#define APRS_TX_INTERVAL  120000  // APRS TX Interval [ms]
...
/*
 ...
 127 20dBm  (100mW max)
*/
  Si446x radio1(RADIO_PIN);   // create the radio object

#elif RADIO == RFM22
  #include "RFM22.h"
  #define RADIO_PIN 10
  #define RADIO_SDN 3      
  #define STATUS_LED 4     // PAVA R7 Boards have an LED on Pin 4
  ...
  /*
   ...
   0x07 20dBm (100mW)
  */
  rfm22 radio1(RADIO_PIN); // create the radio object
#endif
...
void setup() {
  ...
  Serial.begin(9600);
  blinkled(5);
#if RADIO == SI446X  // Here we need to enable the GPS power supply
  pinMode(GPS_POWER_PIN, OUTPUT);
  digitalWrite(GPS_POWER_PIN, HIGH); // leave it on permanently for now;
  pinMode(GPS_RESET_PIN, OUTPUT);
  digitalWrite(GPS_RESET_PIN, HIGH); // leave it on permanently for now;
#endif  
  resetGPS();
  blinkled(4);
  wait(500);
...
/*    
// Not sure what this does??? Not implemented for Si446x!!
#if RADIO == RFM22
...
ISR(TIMER1_COMPA_vect)
{
  ...
  case 1: // Initialise transmission
    if(radiostatus==1)
    {
      ...
      lockvariables=1;
      sprintf(txstring, "$$$$$%s,%i,%02d:%02d:%02d,%s%i.%05ld,%s%i.%05ld,%ld,%d,%i", CALLSIGN, count, hour, minute, second,lat < 0 ? "-" : "",lat_int,lat_dec,lon < 0 ? "-" : "",lon_int,lon_dec, maxalt,sats,errorstatus);
      sprintf(txstring, "%s,%c%c,%i",txstring,comment[0]==' ' ? '-' : comment[0],comment[1]==' ' ? '-' : comment[1],aprs_attempts);
      sprintf(txstring, "%s,%d,%ld,%ld", txstring, bmp085temp, bmp085pressure, bat_mv);
...
void rtty_txbit (int bit)
{
  ...
  if (bit)
  {
    //radio1.start_tx(1); // High => channel 1
    radio1.setHighTone();
  }
  else
  {
    //radio1.start_tx(0); // Low => channel 0
    radio1.setLowTone();
  }
#elif RADIO == RFM22
  if (bit)
...
void setupRadio(){
  ...
  wait(20000); // give the GPS time to get a lock
  radio1.initSPI();
    
  
#if RADIO == SI446X
  //setPwmFrequency(3, 1); // 31250 Hz
  radio1.setFrequency(RADIO_FREQUENCY);
  radio1.setShift(RTTY_SHIFT);
  radio1.setPowerLevel(RADIO_POWER); 
  // all the rest is done within radio1.init();
  radio1.init();
  radio1.ptt_on();
...
void prepare_data() {
  ...
  // Todo: temperature, pressure, voltage
...
void wait(unsigned long delaytime) // Arduino Delay doesn't get CPU Speeds below 8Mhz
{
  ...
  while((basetime+delaytime)>=millis()){
  }
